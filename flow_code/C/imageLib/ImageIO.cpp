...
void ReadFilePGM(CByteImage& img, char* filename)
{
    // Open the file and read the header
    FILE *stream = fopen(filename, "rb");
    if (stream == 0)
...
void WriteFilePGM(CByteImage img, char* filename)
{
...
void ReadImage (CImage& img, char* filename)
{
	if (filename == NULL)
...
void WriteImage(CImage& img, char* filename)
{
	if (filename == NULL)
...
void ReadImageVerb(CImage& img, char* filename, int verbose) {
	if (verbose)
...
void WriteImageVerb(CImage& img, char* filename, int verbose) {
	if (verbose)
